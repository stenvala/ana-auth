        ...
        for method in response["paths"][path].keys():
            ...
            try:
                ...
                if http_code == "204":
                    ...
                    response_type = schema["$ref"].replace("#/components/schemas/", "")
                ...
                if "requestBody" in response["paths"][path][method]:
                    ...
                    if "application/json" in request_body_content:
                        ...
                        # File upload endpoint — extract file and non-file form fields
                        is_multipart = True
                        multipart_schema_ref = request_body_content[
                            "multipart/form-data"
                        ]["schema"].get("$ref", "")
                        multipart_schema_name = multipart_schema_ref.replace(
                            "#/components/schemas/", ""
                        )
                        multipart_props = (
                            response.get("components", {})
                            .get("schemas", {})
                            .get(multipart_schema_name, {})
                            .get("properties", {})
                        )
                        multipart_form_fields = []
                        multipart_file_field_name = "file"
                        for prop_name, prop_def in multipart_props.items():
                            if prop_def.get("format") == "binary" or (
                                prop_def.get("type") == "array"
                                and prop_def.get("items", {}).get("format") == "binary"
                            ):
                                multipart_file_field_name = prop_name
                                parameters.append("file: File")
                            else:
                                camel_name = snake_to_camel(prop_name)
                                prop_type = get_type(prop_def, prop_def)
                                parameters.append(f"{camel_name}: {prop_type}")
                                multipart_form_fields.append((prop_name, camel_name))
                ...
                if method == "get":
                    ...
                    # Generate FormData handling for file uploads + form fields
                    form_data_lines = f"const formData = new FormData(); formData.append('{multipart_file_field_name}', file);"
                    for orig_name, camel_name in multipart_form_fields:
                        form_data_lines += (
                            f" formData.append('{orig_name}', {camel_name});"
                        )
                    api_method += (
                        form_data_lines
...
if __name__ == "__main__":
    ...
    main_app_open_api_url = "http://localhost:6785/openapi.json"
